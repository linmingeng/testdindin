...
if($method){
    $view_file = BASE_PATH."/views/".$controller."_".$method.".php";
    $call_method = $method.'_'.strtolower($_SERVER['REQUEST_METHOD']);
}else{
    $view_file = BASE_PATH."/views/".$controller."_default.php";
    $call_method = strtolower($_SERVER['REQUEST_METHOD']);
}
...
if (file_exists($controller_file)) {
    ...
    if ( ! method_exists($obj, $call_method)) {
        ...
        if (is_callable(array($obj, $call_method))) {
            $ret = $obj -> $call_method();
            $ajax = (int)$_REQUEST['ajax'];     //是否ajax调用
            if (!$ajax && file_exists($view_file)) {
                include $view_file;             //如果不是ajax就直接加载view
            }else{                              //view不存在，则响应json
                if( ! is_array($ret)){          //ajax要
                    if(empty($ret)){
                        $ret = array();
                    }else{
