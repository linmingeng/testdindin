...
class order_module {
    ...
    //获取列表
    function getOrder($appid, $userid = 0, $page = 1,$status=-1){
        global $secret_key;
        ...
        if($status>-1){
            if($status==4){
                array_push($whereArr, "`status` >= $status");
            }else{
                array_push($whereArr, "`status` = $status");
            }
        }
        ...
        $data['pageTotal'] = ceil($data['count']/$this->pageSize);
        if($data["count"]){
...
//                    $goods_id[$order['goods_info']['reduce_goods'][0]['goodsid']] = $order['goods_info']['reduce_goods'][0]['goodsid'];
//                }
//
//                dre($goods_id);
                ...
                foreach($results as $key => &$val){
                    ...
                    $val['add_at'] = date('Y-m-d H:i:s',$val['add_at']);
                }
            }
    ...
    function pcAdd($addressid){
        ...
        $goods = json_decode($_COOKIE['cart'],true);
        foreach($goods as $goodsid=>$quantity){
            //$goods_info["reduce_goods"][$res['goodsid']] = $res['quantity'];
            $goods_info["reduce_goods"][] = array(
                'goodsid' => $goodsid,
                'modelsid' => 0,
            ...
            $goods_quantity[$goodsid] = $quantity;
        }
        $goods_moduel = new goods_module();
        $goods_list = $goods_moduel->getSaleGoods(array_values($goodsids));
        $total_price = 0;
        foreach($goods_list as $val){
            $quantity = $goods[$val['goodsid']];
            $goods_total_price = $val['price']*$quantity;
        ...
        $data = array(
            ...
            "price" => $total_price,                          //订单金额
            "delivery_fee" => 0,            //运费
            "payment" => 0,
            "reach_consume" => 0,          //满减消费额
            "reach_reduce" => 0,            //满减金额
            "reach_reduceid" => 0,
            "newbie_reduce" => 0,          //新用户首单优惠金额
            "newbie_reduceid" => 0,
            "reduce_data" => '',
            "couponid" => 0,
            "coupon_consume" => 0,
        ...
        $orderid = $this->db->add('the_order',$data);   //写入订单
        $goodsids = $this->pcAddOrderGoods($goods_list,$order_num,$goods_quantity);
        return $orderid;
    }
//    PC端添加订单商品
    function pcAddOrderGoods($goods_list,$order_number,$goods_quantity){
        global $user_info;
        foreach($goods_list as $res){
            $data = array(
                ...
                "return_money" => $res['return_money'],
                "modelsid" => $res['modelsid'],
                "quantity" => $goods_quantity[$res['goodsid']],
                "add_at" => time()
            );
        }
        $this->db->add('the_order_goods',$data);    //写入订单商品
