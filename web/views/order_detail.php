      ...
      <div class="info">
			<div class="detail clearfix">
				<div class="col-xs-8 col-sm-8 col-lg-4 dingdan">
                    <?php $order = $ret['order'];?>
					<p>订单号：<span><?php echo $order['order_number'];?></span></p>
				</div>
				<div class="col-xs-4 col-sm-4 col-lg-8 staus">
					<p>状态：<span><?php echo $order['status']?>
					</span></p>
				</div>
			</div>
			<div class="info-text">
				<h2>订单信息</h2>
				<ul>
					<li>下单时间：<span><?php echo $order['add_at']?></span></li>
					<li>支付方式：<span>网上支付</span></li>
					<li>收货信息：<span><?php echo $order['name'].$order['phone'].$order['area'].$order['address'];?></span></li>
					
					<li>物流名称：<span>叮叮物流</span></li>
					<li>物流编号：<span><?php echo $order['delivery_sn'];?></span></li>
				</ul>
			</div>
			<hr>
        ...
        <div class="shop-list">
			...
			<table>
				<tbody>
					...
					<tr>
						<td>
							<img src="'.$order_goods['image'].'" alt="">								
							<p><a title="'.$order_goods['name'].'" href="?goodsDetail/view/goodsid/'.$order_goods['goodsid'].'" class="nameSubStr">'.$order_goods['name'].'</a></p>
						</td>
						<td>￥'.$order_goods['price'].'</td>
						<td>×'.$order_goods['quantity'].'</td>
						<td>--</td>
						<td><span>￥'.$order_goods['total_price'].'</span></td>
