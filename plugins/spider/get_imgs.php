...
$sql = "SELECT `id`,`imgs` FROM `sh_product` WHERE `flag` = 1 ";
$ret = $db->getX($sql);
foreach($ret as $re){
    if($re['imgs']){
        $imgs = json_decode($re['imgs'],1);
        foreach($imgs as $i => $img){
            $imgArr = explode('/',$img);
            //print_r($imgArr[count($imgArr)-1]);exit;
            file_put_contents('shimgs/'.$i.'/'.$imgArr[count($imgArr)-1], file_get_contents($img));
        }
        $sql = "UPDATE `sh_product` SET `flag`=1 WHERE `id` = '".$re['id']."' ";
        $db->updateX($sql);
    }
}
