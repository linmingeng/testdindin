...
class upload {
    ...
    public  $immediate;
    public  $upfile;
    public  $fileType = 'file';                                                         //文件类型：img=>图片,swf=>flash动画,flv=>flv视频,media=>媒体文件,pem=>pem格式证书,other=>其他
    ...
    public  $upext = 'doc,txt,rar,zip,jpg,jpeg,gif,png,swf,flv,wmv,avi,wma,mp3,m4a,mid,pem,p12';    //上传扩展名:doc,txt,rar,zip,jpg,jpeg,gif,png,swf,wmv,avi,wma,mp3,m4a,mid,pem
    ...
    function uploadNow(){
        ...
        if(!empty($upfile['error'])){
            ...
            if($upfile['name'] == 'blob' && $upfile['type'] == 'image/jpeg'){   //支持blob上传图片
                $upfile['name'] = date("YmdHis").mt_rand(1000,9999).'.jpg';
            }
